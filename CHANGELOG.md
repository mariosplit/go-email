...
### Added
- **Calendar operations** via the new `CalendarProvider` interface, implemented
  for **Outlook 365** (Gmail returns `ErrUnsupported`). Purely additive; mail
  and `Send` are unchanged.
  - `ListEvents` — events in a date range (via Graph `calendarView`), paginated.
  - `ReadEvent` — one event including body and attendees.
  - `CreateEvent` / `UpdateEvent` / `DeleteEvent` — POST / PATCH (non-zero
    fields only) / DELETE.
  - New types `Event`, `EventListOptions`; `Client` wrappers plus
    `…WithContext` variants for each.
  - Reads send `Prefer: outlook.timezone="Australia/Perth"` so event times come
    back as AWST wall-clock rather than UTC.
- Requires the **Calendars.ReadWrite** (application) permission on the Azure app
  for the Outlook calendar methods.
